...
def get_thumbnail_fixed(slide, level, size):
    """
    Replacement for slide.get_thumbnail(size) that returns an exact size
    using read_region. size = (width, height)
    """
    width, height = size
    return np.array(
        slide.read_region(
...
def compute_interesting_patches(
    ...
    # tiles_w, tiles_h = tiles.level_tiles[largest_single_level]
    # full_tile = np.zeros((tiles.level_dimensions[largest_single_level][1], tiles.level_dimensions[largest_single_level][0], 3), dtype=np.uint8)
    # for x in range(tiles_w):
    #     for y in range(tiles_h):
    #         tile = tiles.get_tile(largest_single_level, (x, y))
    #         print(tile.size)
    #         x_start = min(x * 1024, full_tile.shape[0] - tile.size[0])
    #         y_start = min(y * 1024, full_tile.shape[1] - tile.size[1])
    #         full_tile[y_start:y_start+tile.size[1], x_start:x_start+tile.size[0], :] = np.array(tile)
    ...
    # full_tile = get_thumbnail_fixed(slide, largest_single_level, tiles.level_dimensions[largest_single_level])
